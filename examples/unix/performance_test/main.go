...
func main() {
	...
	testConfig := &TestConfig{
		TotalTasks:    500000, // 总任务数 / Total number of tasks
...
func createPluginConfigs(examplesDir string) map[string]*config.PluginConfig {
	return map[string]*config.PluginConfig{
		"math": {
			...
			Path:                filepath.Join(examplesDir, "unix", "math_plugin", "math_plugin-linux"),
		...
		"string": {
			Type:                config.PluginTypeBinary,
			Path:                filepath.Join(examplesDir, "unix", "string_plugin", "string_plugin-linux"),
			PoolSize:            10,
			MaxInstances:        10,
			HealthCheckInterval: 30 * time.Second,
		...
		"node": {
			...
			Interpreter:         "node",
			ScriptPath:          filepath.Join(examplesDir, "unix", "node_plugin", "index.js"),
			PoolSize:            10, // 减少脚本插件池大小 / Reduce script plugin pool size
			MaxInstances:        10,
			HealthCheckInterval: 30 * time.Second,
		...
		"python": {
			Type:                config.PluginTypeScript,
			Interpreter:         "python3", // 使用python3确保兼容性 / Use python3 for compatibility
			ScriptPath:          filepath.Join(examplesDir, "unix", "python_plugin", "main.py"),
			PoolSize:            10,
			MaxInstances:        10,
			HealthCheckInterval: 30 * time.Second,
